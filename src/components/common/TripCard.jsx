...
function TripCard({ user_id, contentId, contentTypeId, title, addr1, addr2, areaCode, sigunguCode, firstimage, mapX, mapY}) {
  ...
  const handleLikeClick = async (e) => {
    ...
    if (liked) {
      ...
      result = await setLikesContent(user_id, contentId, contentTypeId,
                                     title, addr1, addr2, areaCode, sigunguCode, firstimage, mapX, mapY);
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col justify-between">
        <div>
          <p className="text-sm text-blue-500">{extractSiGu(addr1)}</p>
